...
@Service
public class PascaOnlineService {
    @Autowired
    ...
    public TxPostReceptionDetail selectOnePostDetail(String postReceptionId) {
        return txPostReceptionDetailCustomRepository.selectOne(
                        "", "txPostReception.id", postReceptionId, true, null, null);
    }
    public GenericSearchWrapper<TxPostReception> searchOposisiAdmin(){
...
//            String f = selectOneTxPostReception("id",txPostReception.getId()).getPostNo();
    ...
    @Transactional
    public void saveOrUpdateGeneralDetail3(TxPostReception txPostReception){
    	
    	TxPostReceptionDetail txPostReceptionDetail = txPostReceptionDetailCustomRepository.selectOne("txPostReception.id", txPostReception.getId());
    	if (txPostReceptionDetail != null) {
    		txPostReceptionDetail.setTxTmGeneral(txTmGeneralCustomRepository.selectOne("id", txPostReception.getApplicationIdListTemp().get(0).getApplicationId()));
            txPostReceptionDetailCustomRepository.saveOrUpdate(txPostReceptionDetail);
    	}        
    }
...
//        txPostReceptionDetail.setTxTmGeneral(txTmGeneralCustomRepository.selectOne("id", txPostReceptionDetail.getTxTmGeneral().getId()));
    ...
    @Transactional
    public void saveOrUpdatePascaStatus(String txPostReception) {
        TxPostReceptionDetail postId = selectOnePostDetail(txPostReception);
        postId.setmStatus(StatusEnum.DONE.value());
        txPostReceptionDetailCustomRepository.saveOrUpdate(postId);
    }
