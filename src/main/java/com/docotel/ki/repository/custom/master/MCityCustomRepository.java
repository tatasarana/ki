...
@Repository
public class MCityCustomRepository extends BaseRepository<MCity> {
	
	public MCity selectOne(String by, Object value) {
        return super.selectOne("LEFT JOIN FETCH c.mProvince p JOIN FETCH c.createdBy cb", by, value, true);
    }
	
    public List<MCity> selectAllByProvince(String by, Object value, Integer offset, Integer limit) {
        return super.selectAll("LEFT JOIN FETCH c.mProvince p", by, value, true, offset, limit);
    }
    
    @Override
	public List<MCity> selectAll(List<KeyValue> searchCriteria, String orderBy, String orderType, Integer offset, Integer limit) {
        try {
            StringBuffer sbQuery = new StringBuffer("SELECT c FROM " + getClassName() + " c ");   
            sbQuery.append("JOIN FETCH c.mProvince mp ");  
